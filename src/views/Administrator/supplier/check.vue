    ...
    <div class='app-container'>
        <el-table :data='list' border fit highlight-current-row>
            ...
            <el-table-column label='操作' width='200'>
                <template slot-scope="scope">
                    <el-button v-if='scope.row.status_examine == 2' type='success' size='mini' @click='check_success(scope.$index)'>通过</el-button>
                    <el-button v-if='scope.row.status_examine == 2' type='danger' size='mini' @click='check_error(scope.$index)'>不通过</el-button>
                </template>
            </el-table-column>
        </el-table>
    </div>
...
<script>
import { check_status, get_audit_data } from '@/api/system'
export default {
  data() {
    return {
      form: {},
